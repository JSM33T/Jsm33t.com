...
export default function BlogListWithSidebar() {
	...
	useEffect(() => { setInput(searchKey); }, [searchKey]);

	useEffect(() => {
		setLoading(true);
		const url = `/blog/list?pageNumber=${page}&pageSize=${pageSize}${searchKey ? `&search=${encodeURIComponent(searchKey)}` : ''}`;
		apiClient.get<BlogListDto[]>(url)
			.then(res => {
				setResults(res.data || []);
				setTotal(Array.isArray(res.data) ? res.data.length : 0); // fallback if no totalCount
			})
